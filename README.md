...
- **`india_sac`** — a continuous-action RL policy (Soft Actor-Critic). Instead of a discrete BUY/SELL it emits an *allocation weight* in [0, 1] per name, trained against a differential Sortino objective net of turnover cost. **Inference only, and no checkpoint ships with the repo** — `load()` fails deliberately when one is absent, because an untrained actor's sigmoid emits arbitrary values and would clear the entry threshold on noise. Requires the `gpu` extra.
