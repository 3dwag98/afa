...
- GJR-GARCH(1,1) conditional volatility with Student-t innovations, used as an optional drop-in replacement for the Monte Carlo simulation's flat historical-volatility assumption (`src/volatility_models.py`; enable via `simulation.use_garch_volatility: true`). Fitting is separated from forecasting and runs on a schedule (`simulation.garch_refit_interval_days`), because one maximum-likelihood fit per ticker per day is ~4.5 million fits for the documented backtest
...
simulation:
  ...
  garch_refit_interval_days: 21 # MLE refits are scheduled; the recursion still runs daily
training:
  walk_forward_splits: 5        # expanding-window validation folds; 0 to skip
  cost_adjusted_target: true    # train on the net move, not the gross one
```
